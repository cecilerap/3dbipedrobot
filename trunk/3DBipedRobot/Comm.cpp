...
// 통신속도 Table
int _nBaudRates[] = { CBR_9600,	CBR_57600, CBR_115200,  };
...
DWORD CommWatchProc(LPVOID lpData)
{
	...
	if ( os.hEvent == NULL )
	{
		MessageBox( NULL, L"Failed to create event for thread!", L"comm Error!",
...
{            
	...
	if (lpTTY->byCommPort > COM_MAXPORTS)
		lstrcpy( szPort, L"\\\\.\\TELNET" ) ;
	else
		wsprintf( szPort, L"COM%d", lpTTY->byCommPort );
